...
class ARDFEventPDF(FPDF):
    def __init__(self, db: Engine, title: str):
    ...
    def table(self, data):
        ...
        def _get_table_widths(data):
            widths = []
            for row in data:
                if row[0] == "CATEGORY" or row[0] == "SPLITS":
                    continue
        ...
        for row in data:
            if row[0] == "CATEGORY":
                self.ln()
                self.set_font("DejaVu", "B", 14)
                self.cell(self.get_string_width(row[1]) + 6, 6, row[1], 0)
                self.set_font("DejaVu", "B", 12)
                self.cell(0, 6, row[2], 0)
                self.ln()
            elif row[0] == "SPLITS":
                self.cell(10, 6, "", 0)
